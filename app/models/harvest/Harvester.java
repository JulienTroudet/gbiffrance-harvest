...
{		
  ...
  public Harvester(Dataset dataset, String targetDirectory, String begin, String end)  
  {
	...
	else LOG.error("Error in harvester selection");
	run(this.dataset);   
  }
  ...
  private void run(Dataset dataset) {
    try {
	  ...
	  if (e.getMessage().equals("Error in range")) LOG.error("Harvester stopped at " + this.currentLower);
  ...
  private void fullHarvest() throws Exception {
    ...
    if (!new File(this.dataset.tempDirectory + "0_null-AAA-0_.txt.gz").exists()||new File(this.dataset.tempDirectory + "zzz-null-0.txt.gz").exists()) {
  ...
  private void partialHarvest() throws Exception {
    ...
    for (char c1 = this.begin.charAt(0); c1<='Z'; c1++) {
	  for (char c2 = this.begin.charAt(1); c2<='z'; c2++) {
		...
		if (c2=='z') this.begin = this.begin.charAt(0) + "" + 'a';
	  }   
	}
	LOG.info("Finished harvesting");
  }
  ...
  public void pageRange(String lower, String upper, int startAt) throws Exception {
    ...
    if (this.dataset.type.equals("biocase")) {
	  ...
	  models.harvest.tapir.ResponseToFileHandler responseToFile = new models.harvest.tapir.ResponseToFileHandler(responseFile);
	  httpClient.execute(httpget, responseToFile);
	} 	    
  ...
  public static void deleteTemporaryDirectory(String directory, String lower) {
	if (directory == null) LOG.info("No temp directory, do nothing");
	else
	{
	  ...
	  if( path.exists() ) {
	    ...
	    for(int i=0; i<files.length; ++i) {
	      if(files[i].isDirectory()) {
	        ...
	        if ((files[i].toString().split(directory)[1].split("-")[0]).compareTo(lower)>= 0 && !(files[i].toString().substring(files[i].toString().length()- 10, files[i].toString().length()).equals("-0_.txt.gz"))) {	        	
	          files[i].delete();   
	        }
	      }
	      else files[i].delete();
	  ...
	  else LOG.info("No temp directory associated to the resource");
      if (path.delete()){
